...
const INTELLIGENCE_RULES = [
  ...
  "• MILESTONE dimension (startup timelines vs enrollment window): Startup uses FSI (Subject In), SIV, Contract, IRB, Site Selected. PSM enrollment window uses FPFV→LPFV visit milestones only — NOT FSI/LSI. LSO/LPLV = treatment end — not LPFV.",
  ...
  " DATE SOURCE (critical): PSM months come from FPFV and LPFV milestone actuals on ora_veeva_milestone — not study header fields.",
  " FPFV = First Patient/Subject First Visit In. LPFV = Last Patient/Subject First Visit In. These are VISIT milestones.",
  " FSI / LSI (First/Last Subject In) are NOT the PSM window — do not substitute FSI for FPFV or LSI for LPFV.",
  " LPLV / LSO = Last Patient Last Visit / Last Subject Out — treatment end, NOT LPFV.",
  " SOURCE 1 — Veeva / Ora site-level (computed live): site_psm = total_enrolled / site_enroll_months.",
  " site_enroll_months = months from site FPFV → site LPFV; if same month, site_enroll_months = 1 (never divide by zero).",
  " When context shows fpfv_date / lpfv_date on a site row, those ARE the PSM window milestone actuals.",
...
function systemPromptFor(context) {
  ...
  if (!user?.firstName && !user?.displayName && !user?.email) {
    return base + protocols + workflowNote + modeNote + pendingTaskNote + routerNote + deptNote + evidenceNote + huntNote + focusNote + moneyNote + intelNote + planNote + visualNote + docNote + fillNote + reconcileNote + liveNote + legacyNote + modelNote;
  }
